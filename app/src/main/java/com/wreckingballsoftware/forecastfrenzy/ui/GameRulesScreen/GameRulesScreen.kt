package com.wreckingballsoftware.forecastfrenzy.ui.GameRulesScreen
...
@Composable
fun GameRulesScreen(navGraph: NavGraph) {
    Column(
        modifier = Modifier.fillMaxWidth(),
        horizontalAlignment = Alignment.CenterHorizontally,
    ) {
        Text(
            text = "Game Rules Screen",
            style = MaterialTheme.forecastTypography.headline
        )
        Spacer(modifier = Modifier.height(64.dp))
        Button(
            onClick = { navGraph.navigateToGameplayScreen() }
        ) {
            Text(text = "Play")
        }
    }
